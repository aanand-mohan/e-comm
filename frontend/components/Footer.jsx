...
export default function Footer() {
    ...
    return (
        <footer className="bg-black text-gray-400 mt-auto border-t border-white/10 font-sans relative z-10">
            {/* Main Footer Content */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
                    ...
                    <div className="space-y-6">
                        <Link href="/" className="inline-block">
                            <div className="flex flex-col">
                                <span className="text-2xl font-serif font-bold text-white tracking-wide">
                                    {!loading && getContent('footer_logo_text', 'RUDRADIVINE')}
                                </span>
                                <span className="text-[0.65rem] tracking-[0.3em] text-gray-400 uppercase">
                        ...
                        <div className="flex items-center gap-4">
                            <a href={!loading ? getContent('social_facebook', '#') : '#'} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary hover:text-black transition-all duration-300">
                                <Facebook size={18} />
                            </a>
                            <a href={!loading ? getContent('social_instagram', '#') : '#'} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary hover:text-black transition-all duration-300">
                                <Instagram size={18} />
                            </a>
                            <a href={!loading ? getContent('social_twitter', '#') : '#'} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary hover:text-black transition-all duration-300">
                                <Twitter size={18} />
                            </a>
                            <a href={!loading ? getContent('social_linkedin', '#') : '#'} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center hover:bg-primary hover:text-black transition-all duration-300">
                                <Linkedin size={18} />
                            </a>
                        </div>
                    </div>
            ...
            <div className="bg-black py-8 border-t border-white/5">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4">
                    <p className="text-sm text-gray-600 text-center md:text-left">
                        {/* Dynamic Copyright */}
                        {!loading && getContent('footer_copyright', '© 2026 Rudra Divine. With Blessings.')}
                    </p>
                    <div className="flex items-center gap-4 opacity-50 hover:opacity-100 transition-opacity duration-300">
                        ...
                        <span className="text-xs uppercase tracking-widest text-gray-700">Secure Payments</span>
                    </div>
                </div>
            </div>
        </footer>
